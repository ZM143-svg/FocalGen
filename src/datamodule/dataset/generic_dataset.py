    ...
    def __getitem__(self, index: int) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        if self.is_test:
            ...
            if len(annos) == 0:
                ...
                keypoints = annos[:, 1:3]
                # 将1维labels转为2维(211,1)
                labels = annos[:, 0].reshape(-1, 1)  # 核心修改：增加reshape
                shapes = annos[:, 3:5]
                # 添加维度一致性检查
                assert len(keypoints) == len(labels) == len(shapes), \
                    f"维度不一致: keypoints={len(keypoints)}, labels={len(labels)}, shapes={len(shapes)}"

        if self.transforms:
            # 训练模式也需要处理
            if not self.is_test:
                keypoints = annos[:, 1:3] if len(annos) > 0 else np.zeros((0, 2))
                labels = annos[:, 0].reshape(-1, 1) if len(annos) > 0 else np.zeros((0, 1))
            transformed = self.transforms(
                image=image,
                keypoints=keypoints,
                labels=labels.squeeze(1) if len(labels) > 0 else labels,
                # shapes=shapes,
            )
