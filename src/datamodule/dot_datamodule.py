...
class DotDatamodule(LightningDataModule):
    def __init__(self,
        ...
        elif self.dataset == 'merged_yolo_pixel':
            from src.datamodule.dataset.merged_pixel_dataset import MergedPixelDataset as DotDataset

            # merged_root = Path('D:/merged_dataset')  # 或从 self.root_data_path 获取
            merged_root = Path(self.root_data_path)
            all_imgs = sorted(merged_root.glob('images/*.jpg'))
        ...
        augmentation = A.Compose([
            ...
            # 修改后
            # A.RandomResizedCrop(size=(self.image_size[1], self.image_size[0]), scale=(0.5, 1.0), ratio=(1.0, 1.0),
            #                     p=0.4),
            A.RandomResizedCrop(size=(self.image_size[1], self.image_size[0]), scale=(0.5, 1.0), ratio=(1.0, 1.0),
                                p=0.4),
            ...
            keypoint_params=A.KeypointParams(format='xy', remove_invisible=True, label_fields=['labels', ])
        )
